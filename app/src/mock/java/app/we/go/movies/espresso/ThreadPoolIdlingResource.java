...
package app.we.go.movies.espresso;
...
public abstract class ThreadPoolIdlingResource implements IdlingResource {
    ...
    public ThreadPoolIdlingResource(ThreadPoolExecutor executor) {
        this.threadPoolExecutor = Preconditions.checkNotNull(executor,
    ...
    @Override public boolean isIdleNow() {
        ...
        if (!threadPoolExecutor.getQueue().isEmpty()) {
            ...
            int activeCount = threadPoolExecutor.getActiveCount();
                    LOG.d(TAG, "Active count: %d", activeCount);
            if (0 != activeCount) {
                if (monitor.get() == null) {
    ...
    void notifyWhenIdle(final Runnable idleCallback) {
        ...
        Preconditions.checkState(monitor.compareAndSet(null, myMonitor), "cannot monitor for idle recursively!");
    ...
    private class IdleMonitor {
        ...
        private IdleMonitor(final Runnable onIdle) {
            this.onIdle = Preconditions.checkNotNull(onIdle);
